...
int main(int argc,char *argv[]){
    ...
    for(nodes=7;nodes<=21;nodes+=7){
        
        hiddenNodes.clear();
        hiddenNodes.push_back(nodes);
        
        for(float LearningRate = 0.4f ; LearningRate <= 0.8f ; LearningRate = LearningRate + 0.3f){
            
            for(iterations=200;iterations<=300;iterations+=100){
